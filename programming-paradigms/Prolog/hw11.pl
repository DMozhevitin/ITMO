...
search(tree((K, _, _), L, _), NK, NV, F) :- NK < K, search(L, NK, NV, F).
search(tree((K, _, _), _, R), NK, NV, F) :- NK > K, search(R, NK, NV, F).
map_get(T, K, V) :- search(T, K, V, F), F == 1.
...
map_put(T, K, V, R) :- rand_int(2147483647, P), search(T, K, _, F), (F == 1 -> replace(T, K, V, R); insert(T, (K, V, P), R)).

map_size(nil, 0).
map_size(tree((_, _, _), L, R), SZ) :- map_size(L, RL), map_size(R, RR), SZ is RL + RR + 1.

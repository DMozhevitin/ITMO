...
pair<vector<int>, vector<int>> createSuffixArray()
{
...
int main()
{
    ...
    auto res = createSuffixArray();
    suffixArray = res.first;
    classes = res.second;
    ...
    if (k)
    {
        ...
        {
            if (classes[suffixArray[index]] != classes[suffixArray[index - 1]]) k--;
    ...
    {
        cout << s.substr(static_cast<size_t>(suffixArray[index]), s.size()) +
        s.substr(0, static_cast<size_t>(suffixArray[index]));
    }
}
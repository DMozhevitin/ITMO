...
int main()
{
    ...
    vector<bool> color(suffixArray.size());
    ...
    int mx = -1, ans = -1;
    for (int i = 2; i < suffixArray.size() - 1; i++)
    {
        if (color[suffixArray[i]] != color[suffixArray[i + 1]] && lcp[i] > mx)
        {
            mx = lcp[i];
            ans = suffixArray[i];
        }
    ...
    if (ans != -1)
    {
        cout << s.substr(static_cast<size_t>(ans), static_cast<size_t>(mx));
    }

...
class HttpServer {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        String uri = URLDecoder.decode(request.getUri(), StandardCharsets.UTF_8.name()).trim();
        ...
        if (file.isFile()) {
            attachFileToResponse(request, file, response);
        } else if (file.isDirectory()) {
            File defaultFile = new File(file.getCanonicalPath(), "index.html");
            if (defaultFile.isFile()) {
                attachFileToResponse(request, defaultFile, response);
            }
        } else {
    ...
    private void attachFileToResponse(Request request, File file, Response response) throws  IOException{
        if (request.getHeader("If-None-Match") != null &&
                request.getHeader("If-None-Match").equals(String.valueOf(file.length() * 228))) {
            // I COULD USE LAST-MODIFIED HERE BUT IT DOESN'T WORK ON CF
            response.setStatusCode(304);
            response.setHeader("ETag", String.valueOf(file.length() * 228));
            return;
        }
